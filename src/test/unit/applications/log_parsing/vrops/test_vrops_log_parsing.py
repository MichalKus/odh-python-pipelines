...
class VROPSParsingTestCase(BaseMultipleMessageParsingTestCase):
    ...
    def test_vrops_single_metric(self):
        self.assert_parsing(
            {
                "source": "VROPS.log",
                "message": "disk,name=lg-l-s-uxp00012,kind=VirtualMachine usage_average=6.5333333015441895 1517835494431\n"
            },
            {
                "group": "disk",
    ...
    def test_vrops_multiple_metric(self):
        self.assert_parsing(
            {
                "source": "VROPS.log",
                "message": "summary,name=TN_DEU_PROD_CF_APP_CF_OBO_01_EPG_CF_ACS_DB_01,kind=DistributedVirtualPortgroup used_num_ports=4.0,max_num_ports=8.0,ports_down_pct=0.0 1517835494431\n"
            },
            {
                "group": "summary",
    ...
    def test_vrops_with_gap(self):
        self.assert_parsing(
            {
                "source": "VROPS.log",
                "message": "resourcelimit,name=vRealize\ Operations\ Manager\ Remote\ Collector-NLCSAPVROPS005C,kind=vC-Ops-Remote-Collector numprocessesmax=192100.0,openfiles=100000.0,numprocesses=192100.0,openfilesmax=100000.0 1517835610321\n"
            },
            {
                "group": "resourcelimit",
    ...
    def test_vrops_random(self):
        self.assert_parsing(
            {
                "source": "VROPS.log",
                "message": "cpu,name=nlcsapesxp010.csa.internal,kind=HostSystem idle_summation=9855.2001953125,used_summation=117.13333129882812,usage_average=0.5806666612625122 1518112552376\n"
            },
            {
                "group": "cpu",
    ...
    def test_rare_case(self):
        self.assert_parsing(
            {
                "source": "VROPS.log",
                "message": "net,name=LG-W-P-VDI10028,kind=VirtualMachine droppedpct=40.50104522705078,packetstxpersec=0.3333333432674408,packetsrxpersec=1.5666667222976685 1518115192381\n"
            },
            {
                "group": "net",
