...
QueryFilter = namedtuple("QueryFilter", "error_message metric_name")
...
class VsppDiagnosticServer(BasicAnalyticsProcessor):
    ...
    def _process_pipeline(self, read_stream):
        query_filters = [
            QueryFilter("error got \d*, expected \d*, packet \d*",
                        "total_count_of_cc_error"),
            QueryFilter("Ingest session service error, failed starting session handler, nested error:",
                        "total_count_of_cc_ingest_session_service_error"),
            QueryFilter("underflow in input ES",
        ...
        count_cc_errors_stream = read_stream \
            .withColumn("message_type",
                        custom_translate_regex(
                            ...
                            mapping=dict(
                                (".*" + query_filter.error_message + ".*", query_filter.metric_name) for query_filter in
                                query_filters),
